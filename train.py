...
def train_fn(disc_R, disc_T, gen_RT, gen_TR, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler, epoch, mlflow):
    disc_R.train()
    disc_T.train()
    gen_TR.train()
    gen_RT.train()
...
def main():
    ...
    if config.LOAD_MODEL:
        load_checkpoint(
            config.CHECKPOINT_GEN_R, gen_TR, opt_gen, config.GEN_LEARNING_RATE,
        )
        load_checkpoint(
            config.CHECKPOINT_GEN_T, gen_RT, opt_gen, config.GEN_LEARNING_RATE,
        )
        load_checkpoint(
            config.CHECKPOINT_DISC_R, disc_R, opt_disc, config.DIS_LEARNING_RATE,
        )
        load_checkpoint(
            config.CHECKPOINT_DISC_T, disc_T, opt_disc, config.DIS_LEARNING_RATE,
        )
    ...
    if config.TRAIN:
        for epoch in range(config.NUM_EPOCHS):
            ...
            if config.SAVE_MODEL:
                ...
                save_checkpoint(gen_RT, opt_gen, filename=config.CHECKPOINT_GEN_T)
                save_checkpoint(disc_R, opt_disc, filename=config.CHECKPOINT_DISC_R)
                save_checkpoint(disc_T, opt_disc, filename=config.CHECKPOINT_DISC_T)
    else:
