...
def train_fn(disc_H, disc_Z, gen_Z, gen_H, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler, epoch):
    H_reals = 0
    H_fakes = 0
    loop = tqdm(loader, leave=True, desc=f"{config.DATASET_NAME} TRAIN: ")
...
def main():
    ...
    dataset = HorseZebraDataset(
        root_horse=config.TRAIN_DIR + f"/{config.REFERENCE_NAME}",
        root_zebra=config.TRAIN_DIR + f"/{config.TARGET_NAME}", transform=config.transforms
    )
    val_dataset = HorseZebraDataset(
        root_horse=config.VAL_DIR + f"/{config.REFERENCE_NAME}", root_zebra=config.TRAIN_DIR + f"/{config.TARGET_NAME}",
